...
TeleopJoy::TeleopJoy() : Node("teleop_joy") {
  ...
  auto joy_callback = [this](Joy::UniquePtr msg) -> void {
    joy_msg_ = *msg;
#ifdef ROS_DISTRO_JAZZY
    cmd_vel_msg_.twist.linear.x =
        joy_msg_.axes[1] * (1.0 - joy_msg_.axes[2]) / 4.0;
    cmd_vel_msg_.twist.angular.z =
        joy_msg_.axes[0] * (1.0 - joy_msg_.axes[2]) / 4.0;
#endif
#ifdef ROS_DISTRO_HUMBLE
