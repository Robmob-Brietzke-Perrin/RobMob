...
PlannerNode::PlannerNode() : Node("rrt_planner_node") {
  ...
  // Abonnement à la Map
  // QoS TransientLocal pour recevoir la map même si publiée avant le lancement
  // du node
  ...
  compute_path_server_ = rclcpp_action::create_server<ComputePath>(
      this, "compute_path", std::bind(&PlannerNode::handle_goal, this, _1, _2),
...
void PlannerNode::handle_accepted(
    const std::shared_ptr<GoalHandleComputePath> goal_handle) {
...
void PlannerNode::execute(
  ...
  for (; iteration < max_iterations_ && rclcpp::ok(); ++iteration) {
    ...
    if (rrt_connect.extend(*tree_a, q_rand, q_new_node) !=
        RRTConnect::TRAPPED) {
      std::shared_ptr<TreeNode> q_connect_node;
      if (rrt_connect.connect(*tree_b, q_new_node->get_pose(),
                              q_connect_node) == RRTConnect::REACHED) {
        // Reconstitution du chemin
        if (tree_a == &tree_start)
          final_path = rrt_connect.get_path(q_new_node, q_connect_node);
        else
          final_path = rrt_connect.get_path(q_connect_node,
                                            q_new_node);  // Inversion si swap

        success = true;
        break;
      }
    }
    // TODO: reduce jittering: try skipping points iteratively -> less points,
    // direct, but sharp turns
    // -> Send traj as first valid in feedback
    // -> Continue action to try to smooth out the path with elastic band =
    // minimiser longueur chemin + maximiser distance aux obstacles en
    // régression linéaire.
  ...
  if (success) {
    ...
    RCLCPP_WARN(this->get_logger(), "Failed to find path after %d iterations",
                iteration);
    result->success = false;
    goal_handle->abort(result);
  }
...
ros2 run nav2_map_server map_server --ros-args -p
yaml_filename:=/home/joachimperrin/turtlebot4_env_docker/shared/robmob_ws/src/robmob_bringup/config/test_map.yaml

ros2 lifecycle set /map_server configure

ros2 run rrt_connect_planner planner_node

ros2 action send_goal /compute_path robmob_interfaces/action/ComputePath
"{start: {header: {frame_id: 'map'}, pose: {position: {x: -9.0, y: -9.0, z:
0.0}, orientation: {w: 1.0, x: 0.0, y: 0.0, z: 0.0}}}, goal: {header: {frame_id:
'map'}, pose: {position: {x: 40.0, y: 40.0, z: 0.0}, orientation: {w: 1.0, x:
0.0, y: 0.0, z: 0.0}}}}"
