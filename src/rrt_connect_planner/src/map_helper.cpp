...
// ---------------------------------------------------------
// Méthodes de vérification (Core Logic)
// ---------------------------------------------------------
...
void MapHelper::inflate_obstacles(float bot_radius)
{
  ...
  // Copie des données originales pour ne pas "étaler" l'inflation indéfiniment
  ...
  for (int y = 0; y < h; ++y)
  {
    for (int x = 0; x < w; ++x)
    {
      ...
      // Si la cellule originale est un obstacle
      if (original_data[idx] > 0) // On considère > 0 comme obstacle
      {
        // On remplit le carré/cercle autour
        for (int dy = -cell_radius; dy <= cell_radius; ++dy)
        {
          for (int dx = -cell_radius; dx <= cell_radius; ++dx)
          {
            // Optimisation: Cercle plutôt que carré
