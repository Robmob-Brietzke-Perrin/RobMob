...
std::shared_ptr<TreeNode> RRTConnect::nearest_neighbor(const Point &point, const Tree &tree) {
    ...
    for (const auto &node : tree.get_nodes()) {
        // Comparing squared dist is enough
...
std::vector<Point> RRTConnect::smooth_corners(const std::vector<Point>& path, double max_cut_dist) {
    ...
    smooth_path.reserve(path.size() * 2); // Pré-allocation pour la performance
    ...
    for (size_t i = 1; i < path.size() - 1; ++i) {
        ...
        // Sécurité : Si les points sont déjà trop proches, on ne lisse pas ce virage
        if (dist_BA < 0.05 || dist_BC < 0.05) {
            smooth_path.push_back(B);
            continue;
        }

        // On adapte d dynamiquement à l'espace disponible
        // On ne coupe jamais plus de 40% d'un segment pour ne pas "manger" le virage suivant
