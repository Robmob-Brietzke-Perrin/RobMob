...
class MapHelper {
 ...
 public:
  enum State {
    ...
    INITIALIZED,
    INFLATED
  };  // gives the various states of the map

  void initialize(
      const OccupancyGrid &map_msg);         // sets the map based on the SLAM
  void inflate_obstacles(float bot_radius);  // makes the obstacles bigger to
                                             // make sure we don't hit any

  State get_state() const {
    return state_;
  }  // gives the current state of the map
  const OccupancyGrid &get_map() const {
    return map_;
  }  // returns the current map

  bool is_in_map(const Pose &pose);  // tests if the given point is bounds
  bool is_free(
      const Pose &pose);  // test if the given point isn't in an obstacle

  // Workspace <-> Map conversions
  bool ws_to_map(double wx, double wy, int &mx,
                 int &my);  // converts from workspace to map
  void map_to_ws(int mx, int my, double &wx,
                 double &wy);  // converts from map to workspace
 ...
 private:
  int get_index(int mx, int my)
      const;  // returns the index of the point in the occupancygrid
